...
export default function JobHistoryScreen() {
  ...
  const fetchJobs = async (p: number) => {
    ...
    try {
      const { data } = await api.get('/drivers/jobs', { params: { page: p, limit: 20 } })
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <FlatList
        ...
        onEndReached={() => { if (hasMore && !loadingMore) fetchJobs(page + 1) }}
        onEndReachedThreshold={0.3}
        ListFooterComponent={loadingMore ? <ActivityIndicator color={Colors.brand} style={{ padding: Spacing.lg }} /> : null}
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <View style={s.jobCard}>
              ...
              <View style={s.jobRoute}>
                <View style={s.routeRow}>
                  ...
                  <Text style={s.routeText} numberOfLines={1}>{item.pickupAddress}</Text>
                </View>
                <View style={s.routeLine} />
                <View style={s.routeRow}>
                  <View style={[s.dot, { backgroundColor: Colors.danger }]} />
                  <Text style={s.routeText} numberOfLines={1}>{item.dropoffAddress}</Text>
                </View>
              </View>
              <View style={s.jobBottom}>
                <Text style={s.jobDate}>{format(new Date(item.createdAt), 'dd MMM yyyy · HH:mm')}</Text>
...
const styles = (C: ReturnType<typeof import('../lib/ThemeContext').useTheme>['Colors']) => StyleSheet.create({
  ...
  jobCard: { backgroundColor: C.card, borderRadius: Radius.lg, borderWidth: 1, borderColor: C.border, padding: Spacing.md },
  jobTop: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: Spacing.sm },
  jobRef: { fontSize: FontSize.xs, color: C.brand, fontWeight: '700', fontFamily: 'monospace' },
  ...
  routeLine: { width: 2, height: 12, backgroundColor: C.border, marginLeft: 3, marginVertical: 2 },
  jobBottom: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  jobDate: { fontSize: FontSize.xs, color: C.muted },
  typeBadge: { backgroundColor: C.border, borderRadius: Radius.full, paddingHorizontal: Spacing.sm, paddingVertical: 2 },
