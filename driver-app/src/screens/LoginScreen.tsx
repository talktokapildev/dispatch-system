...
export default function LoginScreen({ onLogin }: { onLogin?: (token: string) => void }) {
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (user.role !== 'DRIVER') {
        Alert.alert('Access Denied', 'This app is for drivers only. Please contact your operator.')
        return
      }
          ...
          {step === 'phone' ? (
            <>
              ...
              <TextInput
                ...
                keyboardType="phone-pad" placeholder="+447123456789"
                placeholderTextColor={Colors.muted} autoFocus
              />
              <Text style={s.hint}>Enter the phone number registered with your operator</Text>
              <TouchableOpacity style={[s.btn, loading && s.btnDisabled]} onPress={sendOtp} disabled={loading}>
                {loading ? <ActivityIndicator color="#000" /> : <Text style={s.btnText}>Send Code →</Text>}
              </TouchableOpacity>
            </>
          ) : (
            <>
              ...
              <TextInput
                ref={otpRef} style={[s.input, s.otpInput]} value={otp}
              ...
              <TouchableOpacity style={[s.btn, loading && s.btnDisabled]} onPress={verifyOtp} disabled={loading}>
                {loading ? <ActivityIndicator color="#000" /> : <Text style={s.btnText}>Verify & Login</Text>}
              </TouchableOpacity>
...
const styles = (C: ReturnType<typeof import('../lib/ThemeContext').useTheme>['Colors']) => StyleSheet.create({
  ...
  logoBox: {
    ...
    backgroundColor: C.brand, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.md,
  },
  ...
  input: {
    ...
    borderRadius: Radius.md, padding: Spacing.md, color: C.text,
