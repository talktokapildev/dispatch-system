...
import { SectionHeader, Table, Modal, EmptyState, Spinner } from '@/components/ui'
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="card overflow-hidden">
        {isLoading ? (
          ...
          <Table
            headers={['Company', 'Contact', 'Email', 'Payment Terms', 'Credit Limit', 'Passengers', 'Jobs']}
              ...
              <tr key={a.id} className="table-row cursor-pointer" onClick={() => setSelected(a)}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-medium text-white">{a.companyName}</p>
                      <p className="text-[10px] text-slate-500">{a.address}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-xs text-slate-300">{a.contactName}</td>
                <td className="px-4 py-3 text-xs text-slate-500">{a.contactEmail}</td>
                <td className="px-4 py-3 text-xs text-slate-400">{a.paymentTermsDays} days</td>
                <td className="px-4 py-3 text-xs font-medium text-brand-400">£{a.creditLimit.toFixed(2)}</td>
                <td className="px-4 py-3 text-xs text-slate-400">{a._count?.passengers ?? 0}</td>
                <td className="px-4 py-3 text-xs text-slate-400">{a._count?.bookings ?? 0}</td>
              </tr>
            ))}
          </Table>
        )}
      ...
      {/* Create modal */}
      <Modal open={creating} onClose={() => setCreating(false)} title="New Corporate Account">
        <form onSubmit={handleSubmit((d) => createMutation.mutate(d))} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { name: 'invoicingEmail', label: 'Invoicing Email', type: 'email' },
            ...
            <div className="col-span-2">
              ...
              <input {...register('address', { required: true })} className="input" placeholder="Full address" />
            </div>
          </div>
