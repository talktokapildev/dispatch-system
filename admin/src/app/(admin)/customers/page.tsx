...
export default function CustomersPage() {
  ...
  const [adjustType, setAdjustType] = useState<"promo" | "real">("promo");
  ...
  const { data: walletData, isLoading: walletLoading } = useQuery({
    queryKey: ["passenger-wallet", selected?.id],
    queryFn: () =>
      api
        .get(`/admin/passengers/${selected.id}/wallet`)
        .then((r) => r.data.data),
    enabled: !!selected?.id,
  });
  ...
  const passengers = (data?.passengers ?? []).filter(
    (p: any) =>
      !search ||
          ...
          >
            {passengers.map((p: any) => (
              <tr key={p.id} className="table-row">
                ...
                <td
                  className="px-4 py-3 text-xs font-mono text-slate-400 cursor-pointer"
                  onClick={() => setSelected(p)}
                >
                  {p.user?.phone}
                </td>
                ...
                >
                  {p.lastBooking
                    ? format(new Date(p.lastBooking), "dd MMM yyyy")
                    : "—"}
                </td>
                <td
                  className="px-4 py-3 text-xs text-slate-400 cursor-pointer"
                  onClick={() => setSelected(p)}
                >
                  {p.totalRides}
                </td>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  label: "Account Type",
                  value: selected.corporateAccountId ? "Corporate" : "Personal",
                },
                { label: "Total Rides", value: selected.totalRides },
                {
                  label: "Stripe Customer",
                  value: selected.stripeCustomerId ? "✓ Set up" : "Not set up",
                },
                {
                  label: "Verified",
            ...
            {selected.savedAddresses?.length > 0 && (
              <div>
                ...
                {selected.savedAddresses.map((addr: any, i: number) => (
                  <div
                    key={i}
                    className="text-xs text-slate-400 p-2 rounded bg-[var(--card-hover)] mb-1"
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-[var(--card-hover)] border border-[var(--border)]">
                    ...
                    <p
                      ...
                      style={{ color: "var(--text)" }}
                    >
                      £{walletData.promoBalance?.toFixed(2)}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-[var(--card-hover)] border border-[var(--border)]">
                    <p className="text-[10px] text-slate-500 mb-1">
                    ...
                    <p
                      ...
                      style={{
                        color:
                          walletData.realBalance < 0
                            ? "#f87171"
                            : "var(--text)",
                      }}
                    >
                      £{walletData.realBalance?.toFixed(2)}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-500">Unable to load wallet</p>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
              ...
              <p className="text-xs text-slate-400">
                ...
                and all their data. Active bookings will prevent deletion.
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setAdjustType("promo")}
                  className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${
                    adjustType === "promo"
                      ? "bg-blue-500 text-white"
                      : "bg-[var(--card-hover)] text-slate-400"
                  }`}
                >
                  Promo credit
                </button>
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1.5 block">
                Amount (use a negative number to deduct, e.g. -5.00)
              </label>
              <input
                type="number"
                step="0.01"
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-400 disabled:opacity-50 text-white text-sm font-semibold transition-colors"
              >
