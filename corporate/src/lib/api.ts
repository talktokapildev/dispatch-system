// corporate/src/lib/api.ts (or wherever your corporate api.ts lives)
...
api.interceptors.request.use((config) => {
  const token =
    typeof window !== "undefined" ? localStorage.getItem("corp_token") : null;
  ...
  persist(
    (set) => ({
      ...
      login: async (email, password) => {
        const { data } = await api.post("/corporate/auth/login", {
          email,
          password,
        });
