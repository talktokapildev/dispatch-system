...
export default function RideHistoryScreen({ navigation }: any) {
  ...
  const fetchBookings = async (p: number) => {
    ...
    try {
      const { data } = await api.get('/passengers/bookings', { params: { page: p, limit: 20 } })
  ...
  if (loading) {
    return (
      <SafeAreaView style={s.container}>
        <View style={s.header}><Text style={s.title}>Ride History</Text></View>
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <FlatList
        ...
        onEndReached={() => { if (hasMore && !loadingMore) fetchBookings(page + 1) }}
            ...
            <View style={s.card}>
              <View style={s.cardTop}>
                ...
                <View style={[s.statusBadge, { backgroundColor: statusColor + '20', borderColor: statusColor + '40' }]}>
                  <Text style={[s.statusText, { color: statusColor }]}>{item.status.replace(/_/g, ' ')}</Text>
                </View>
              </View>
              ...
              <View style={s.cardBottom}>
                <Text style={s.date}>{format(new Date(item.createdAt), 'dd MMM yyyy · HH:mm')}</Text>
                {fare > 0 && (
...
const styles = (C: ReturnType<typeof import('../lib/ThemeContext').useTheme>['Colors']) =>
  StyleSheet.create({
    ...
    ref:         { fontSize: FontSize.xs, color: C.brand, fontWeight: '700', fontFamily: 'monospace' },
