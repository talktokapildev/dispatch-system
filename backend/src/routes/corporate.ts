...
import { BookingStatus, BookingType, PaymentMethod, PricingType } from '@prisma/client'
...
const generateRef = () =>
  `CR${Date.now().toString(36).toUpperCase()}${Math.random().toString(36).slice(2, 5).toUpperCase()}`
...
export async function corporateRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/corporate/auth/login', async (request, reply) => {
    ...
    if (user.role !== 'CORPORATE_ADMIN' && user.role !== 'PASSENGER') {
      return reply.status(403).send({ success: false, error: 'Not a corporate account' })
    }

    if (!user.passenger?.corporateAccount) {
      return reply.status(403).send({ success: false, error: 'No corporate account linked' })
    }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      await fastify.prisma.user.update({ where: { id: user.id }, data: { passwordHash: hash } })
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!corpAccount) {
        return reply.status(403).send({ success: false, error: 'No corporate account' })
      }

      const now = new Date()
      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)

      const [monthlyBookings, upcomingCount, recentBookings] = await Promise.all([
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const isGatwickPickup   = body.pickupAddress.toLowerCase().includes('gatwick')
      const isGatwickDropoff  = body.dropoffAddress.toLowerCase().includes('gatwick')
      const isHeathrowPickup  = body.pickupAddress.toLowerCase().includes('heathrow')
      const isHeathrowDropoff = body.dropoffAddress.toLowerCase().includes('heathrow')
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const corpAccount = await getCorpAccount(request.user.userId)
      if (!corpAccount) return reply.status(403).send({ success: false, error: 'No corporate account' })
  ...
  fastify.patch(
    ...
    async (request, reply) => {
      ...
      const cancellable = [
        BookingStatus.PENDING, BookingStatus.CONFIRMED, BookingStatus.DRIVER_ASSIGNED,
      ]
      ...
      await fastify.prisma.$transaction([
        fastify.prisma.booking.update({
          where: { id },
          data: { status: BookingStatus.CANCELLED, cancellationReason: 'Cancelled by corporate account' },
        }),
        fastify.prisma.bookingStatusHistory.create({
          data: { bookingId: id, status: BookingStatus.CANCELLED, note: 'Cancelled by corporate account' },
        }),
      ])

      fastify.io.to('admin').emit('admin:booking_updated', { bookingId: id, status: 'CANCELLED' })
