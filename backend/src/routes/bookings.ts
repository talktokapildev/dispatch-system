...
import { z } from 'zod'
import { BookingType, BookingStatus, PaymentMethod, PricingType } from '@prisma/client'
...
const generateRef = () =>
  `DS${Date.now().toString(36).toUpperCase()}${Math.random().toString(36).slice(2, 5).toUpperCase()}`
...
export async function bookingRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/bookings/quote', async (request, reply) => {
    ...
    const estimate = await pricing.estimateFare({
      ...
      isAirportPickup: body.type === BookingType.AIRPORT_PICKUP,
      isAirportDropoff: body.type === BookingType.AIRPORT_DROPOFF,
  ...
  // ─── Create booking ───
  fastify.post('/bookings', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const body = createBookingSchema.parse(request.body)
    const { userId } = request.user

    const passenger = await fastify.prisma.passenger.findUnique({ where: { userId } })
    if (!passenger) return reply.status(404).send({ success: false, error: 'Passenger not found' })
    ...
    const booking = await fastify.prisma.booking.create({
      data: {
        ...
        flightArrivalTime: body.flightArrivalTime ? new Date(body.flightArrivalTime) : null,
    ...
    if (!body.scheduledAt) {
      const dispatch = new DispatchService(fastify.prisma, fastify.redis, fastify.io, maps)
      dispatch.dispatchBooking(booking.id).catch((err) =>
        fastify.log.error('Dispatch error:', err)
      )
    }
  ...
  // ─── Get booking by ID ───
  fastify.get('/bookings/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
  ...
  fastify.get('/bookings/my', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const { userId } = request.user
    const { page = '1', limit = '20', status } = request.query as Record<string, string>

    const passenger = await fastify.prisma.passenger.findUnique({ where: { userId } })
    if (!passenger) return reply.status(404).send({ success: false, error: 'Not found' })
    ...
    const [items, total] = await Promise.all([
      fastify.prisma.booking.findMany({
        where,
        include: { driver: { include: { user: true, vehicle: true } }, receipt: true },
    ...
    return reply.send({
      success: true,
      data: { items, total, page: parseInt(page), limit: parseInt(limit), totalPages: Math.ceil(total / parseInt(limit)) },
    })
  })

  // ─── Cancel booking ───
  fastify.patch('/bookings/:id/cancel', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const { id } = request.params as { id: string }
    ...
    const cancellable: BookingStatus[] = [BookingStatus.PENDING, BookingStatus.CONFIRMED, BookingStatus.DRIVER_ASSIGNED]
    if (!cancellable.includes(booking.status as BookingStatus)) {
      return reply.status(400).send({ success: false, error: 'Booking cannot be cancelled at this stage' })
    }
    ...
    await fastify.prisma.$transaction([
      ...
      fastify.prisma.bookingStatusHistory.create({
        data: { bookingId: id, status: BookingStatus.CANCELLED, note: reason },
      }),
    ])

    fastify.io.to(`booking:${id}`).emit(SocketEvent.BOOKING_CANCELLED, { bookingId: id })
    fastify.io.to('admin').emit(SocketEvent.ADMIN_BOOKING_UPDATED, { bookingId: id, status: BookingStatus.CANCELLED })

    return reply.send({ success: true, message: 'Booking cancelled' })
  })

  // ─── Admin: all bookings ───
  fastify.get('/admin/bookings', { preHandler: [fastify.authenticateAdmin] }, async (request, reply) => {
    const { page = '1', limit = '50', status, driverId, from, to } = request.query as Record<string, string>
  ...
  fastify.post('/admin/bookings', { preHandler: [fastify.authenticateAdmin] }, async (request, reply) => {
    const body = createBookingSchema.extend({
    ...
    let passenger = body.passengerId
      ? await fastify.prisma.passenger.findUnique({ where: { id: body.passengerId } })
      : body.passengerPhone
...
if (!passenger && body.passengerPhone) {
  ...
  if (existingUser) {
    ...
    if (!passenger) return reply.status(400).send({ success: false, error: 'Passenger not found' })
    ...
    const booking = await fastify.prisma.booking.create({
      data: {
        ...
        stops: body.stops,
        scheduledAt: body.scheduledAt ? new Date(body.scheduledAt) : null,
        allocationTime: body.allocationTime ? new Date(body.allocationTime) : null,
        paymentMethod: body.paymentMethod,
        pricingType: body.pricingType,
    ...
    if (!body.scheduledAt) {
      const dispatch = new DispatchService(fastify.prisma, fastify.redis, fastify.io, maps)
      dispatch.dispatchBooking(booking.id).catch((err) => fastify.log.error(err))
    }
