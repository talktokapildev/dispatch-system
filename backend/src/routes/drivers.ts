...
export async function driverRoutes(fastify: FastifyInstance) {
      ...
      const body = z
        .object({
          ...
          vehicle: z.object({
            ...
            phvLicenceNumber: z.string().optional(), // ← add
            phvLicenceExpiry: z.string().optional(), // ← add
            phvDiscNumber: z.string().optional(), // ← add
          }),
        })
        .parse(request.body);
        ...
        if (body.vehicle && driver.vehicle) {
          await tx.vehicle.update({
            ...
            data: {
              ...
              ...(body.vehicle.colour && { colour: body.vehicle.colour }),
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      await fastify.prisma.$transaction(async (tx) => {
        // Clear driver from bookings (don't delete booking history)
